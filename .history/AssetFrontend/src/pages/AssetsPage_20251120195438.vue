...
async function addAsset() {
    const res = await axios.post("http://localhost:5011/api/assets", newAsset.value);

    assets.value.push(res.data); // update table  
    showForm.value = false;      // close form
    newAsset.value = { name: "", category: "", assignedTo: "", status: "", purchaseDate: "" }; // reset
}
